...
│                        Entry Points                                  │
│                                                                      │
│   CLI (cli.ts)           HTTP API (server.ts / app.ts)               │
│        │                         │                                   │
│        └──────────┬──────────────┘                                   │
│                   ▼                                                  │
│         ┌─────────────────┐                                          │
│         │   run.ts         │  Core Orchestration Hub                 │
│         └────────┬────────┘                                          │
│                  │                                                   │
│       ┌──────────┼──────────┐                                        │
│       ▼          ▼          ▼                                        │
...
User Prompt
    │
    ▼
selectWorkflow()  ──► AWS Bedrock (Claude 3.5 Sonnet)
    │                       │
    │     BYPASS_BEDROCK=true│
    │◄──── localSemanticFallback() (keyword match)
    │
    ▼
workflowRegistry.execute()
    │
    ▼
DSL Flow  ──► POM Pages  ──► WebdriverIO  ──► Browser (Chrome)
    │
    ▼
WorkflowResult { status, summary, steps[], screenshotPath? }
    │
    ▼
generateReport()  ──► Console / HTTP Response
```
